// parser.ts
import { ParsedCoderef } from './types'; // Assumes types.ts is in the same directory
    ...
    try {
      ...
      for (const pair of pairs) {
        ...
        if (key && value !== undefined) {
           ...
           else if (!isNaN(Number(value)) && value.trim() !== '') metadata[key] = Number(value);
           // Handle quoted strings in fallback
           else if (value.startsWith('"') && value.endsWith('"')) metadata[key] = value.slice(1, -1);
           else if (value.startsWith("'") && value.endsWith("'")) metadata[key] = value.slice(1, -1);
           else metadata[key] = value; // Treat as string
        }
      }
    }
  }
