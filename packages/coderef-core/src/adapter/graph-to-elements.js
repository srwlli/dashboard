 ...
 * ```
 */
export function convertGraphToElements(graph, options = {}) {
    ...
    for (const [id, node] of graph.nodes) {
        ...
        for (const edge of nodeEdges) {
            if (edge.type === 'calls') {
                ...
                if (targetNode) {
                    ...
                    const targetName = lastColonIndex !== -1 ? edge.target.substring(lastColonIndex + 1) : edge.target;
        ...
        if (node.metadata?.parameters && Array.isArray(node.metadata.parameters)) {
            element.parameters = node.metadata.parameters;
        }
        if (calls.length > 0) {
            element.calls = calls;
        }
 ...
 */
export function getConversionStats(elements) {
    const stats = {
...
//# sourceMappingURL=graph-to-elements.js.map