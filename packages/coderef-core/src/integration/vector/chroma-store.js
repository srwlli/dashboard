...
export class ChromaStore {
    ...
    constructor(config) {
        if (!config.indexName) {
            throw new VectorStoreError('Collection name (indexName) is required for Chroma', VectorStoreErrorCode.INVALID_QUERY);
        }
    ...
    async upsert(records, namespace) {
        if (!this.collection) {
            throw new VectorStoreError('Chroma store not initialized. Call initialize() first', VectorStoreErrorCode.CONNECTION_ERROR);
        }
        if (records.length === 0) {
            return;
        }
    ...
    handleError(error) {
        ...
        if (error.message?.includes('not found') || error.message?.includes('does not exist')) {
            return new VectorStoreError(`Chroma collection not found: ${this.collectionName}`, VectorStoreErrorCode.INDEX_NOT_FOUND, error);
        }
        ...
        if (error.message?.includes('connection') ||
            ...
            return new VectorStoreError('Connection error to Chroma. Make sure Chroma server is running.', VectorStoreErrorCode.CONNECTION_ERROR, error);
        }
        // Unknown error
        return new VectorStoreError(`Chroma error: ${error.message}`, VectorStoreErrorCode.UNKNOWN, error);
    }
}
/**
