...
export class PromptTemplateBuilder {
    ...
    buildQAPrompt(variables) {
        ...
        const additionalInstructions = variables.additionalInstructions ||
     ...
     */
    buildConversationalPrompt(variables) {
        let prompt = CONVERSATIONAL_PROMPT_TEMPLATE;
        prompt = prompt.replace('{{question}}', variables.question);
        prompt = prompt.replace('{{context}}', variables.context);
        prompt = prompt.replace('{{conversationHistory}}', variables.conversationHistory || 'No previous conversation.');
        return prompt;
    }
    /**
     * Build code explanation prompt
     */
    buildExplanationPrompt(variables) {
     ...
     */
    buildComparisonPrompt(variables) {
     ...
     */
    buildBestPracticesPrompt(variables) {
     ...
     */
    buildInstructionsFromPreferences(preferences) {
        if (!preferences) {
