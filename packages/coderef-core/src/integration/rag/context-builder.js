...
export class ContextBuilder {
     ...
     */
    buildSummary(results, totalResults) {
    ...
    buildTableOfContents(results) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            lines.push(`${i + 1}. [${result.metadata.type} \`${result.metadata.name}\`](#${anchor}) - ${result.metadata.file}`);
        }
        lines.push('');
        return lines.join('\n');
    }
     ...
     */
    buildResultSection(result, index, options) {
        const sections = [];
        // Header with anchor
     ...
     */
    buildCompactContext(results) {
        const lines = [
            '# Relevant Code\n'
        ];
        for (let i = 0; i < results.length; i++) {
            const result = results[i];
            lines.push(`${i + 1}. \`${result.coderef}\` - ${result.metadata.type} in ${result.metadata.file}`);
            if (result.metadata.documentation) {
     ...
     */
    buildWithTemplate(results, template) {
        const sections = results.map((result, index) => template(result, index + 1));
        return sections.join('\n\n---\n\n');
    }
    /**
     ...
     */
    truncateToTokenLimit(context, maxTokens) {
    ...
    validateContext(context) {
        ...
        if (context.estimatedTokens > 8000) {
            warnings.push(`Context is very large (${context.estimatedTokens} tokens) - may exceed model limits`);
        }
        ...
        // Check diversity
        const uniqueFiles = new Set(context.includedResults.map((r) => r.metadata.file)).size;
        if (uniqueFiles === 1 && context.resultCount > 5) {
            warnings.push('All results from same file - may lack diversity');
        }
        // Check relevance
        const lowScoreCount = context.includedResults.filter((r) => r.score < 0.5).length;
