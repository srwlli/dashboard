...
export class RAGConfigLoader {
    ...
    getLLMProvider() {
        ...
        throw new ConfigError(`LLM provider not configured. Set ${ENV_VARS.LLM_PROVIDER} to 'openai' or 'anthropic', ` +
    ...
    getLLMConfig(provider) {
        if (provider === 'openai') {
            ...
            if (!apiKey) {
                throw new ConfigError(`OpenAI API key not found. Set ${ENV_VARS.OPENAI_API_KEY}`);
            }
            return {
                apiKey,
            ...
            if (!apiKey) {
                throw new ConfigError(`Anthropic API key not found. Set ${ENV_VARS.ANTHROPIC_API_KEY}`);
            }
            return {
                apiKey,
    ...
    getVectorStoreProvider() {
        ...
        if (provider === 'pinecone' || provider === 'chroma') {
            return provider;
        }
        ...
        // Default to Chroma (can run locally)
        return 'chroma';
    }
    /**
     * Get vector store configuration
     */
    getVectorStoreConfig(provider) {
        if (provider === 'pinecone') {
            const apiKey = process.env[ENV_VARS.PINECONE_API_KEY];
            if (!apiKey) {
                throw new ConfigError(`Pinecone API key not found. Set ${ENV_VARS.PINECONE_API_KEY}`);
            }
    ...
    getIndexingSettings() {
        ...
        return {
            batchSize: parseInt(process.env[ENV_VARS.BATCH_SIZE] || String(defaults.batchSize), 10),
    ...
    getSearchSettings() {
        ...
        return {
            defaultTopK: parseInt(process.env[ENV_VARS.DEFAULT_TOP_K] || String(defaults.defaultTopK), 10),
    ...
    getGenerationSettings() {
        ...
        return {
            temperature: parseFloat(process.env[ENV_VARS.TEMPERATURE] || String(defaults.temperature)),
            maxTokens: parseInt(process.env[ENV_VARS.MAX_TOKENS] || String(defaults.maxTokens), 10),
...
${ENV_VARS.VECTOR_STORE_PROVIDER}=chroma
