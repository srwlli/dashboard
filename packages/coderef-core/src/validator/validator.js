...
export class CodeRefValidator {
    ...
    // Extended types
    extendedTypes = new Set(['ML', 'DB', 'SEC']);
    ...
    // Valid status values
    validStatusValues = new Set([
        'active', 'deprecated', 'experimental', 'legacy', 'stable'
    ]);
    // Valid scope values
    validScopeValues = new Set([
    ...
    validate(parsed) {
        ...
        if (!this.isValidTypeDesignator(parsed.type)) {
            ...
            if (this.generateSuggestions) {
                suggestions.push(`Valid types: ${Array.from(this.validTypes).join(', ')}`, `Extended types: ${Array.from(this.extendedTypes).join(', ')}`);
            }
        }
        // Validate path
 ...
 */
export function validateCodeRef(parsed, options) {
 ...
 */
export function validateCodeRefs(parsed, options) {
    const v = new CodeRefValidator(options);
