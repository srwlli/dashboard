...
└── Tree Mode
    ├── Access Mode Indicator (Local/API badge)
...
- **Structure:** Nested menu with projects → directories → subdirectories (max depth 5)
...
## Add to Board Context Menu Integration (WO-FILE-BOARD-CTX-001)

**Added:** 2026-01-19 (v1.2.0)

**Overview:** FileTreeNode now includes "Add to Board" menu item in the right-click context menu, enabling users to add files to Project Boards with 3-5 level nested navigation.

**Integration Point:** `FileTreeNode.tsx` (+61 lines)
...
Right-click file → Context Menu
  └── "Add to Board" (Layers icon)
        ├── Add as New Board
        ├── Add as New List → [Board Picker]
        ├── Add as New Card → [Board Picker] → [List Picker]
        └── Add to Existing Card → [Board Picker] → [List Picker] → [Card Picker]
```
...
2. **State Added:**
```typescript
const [addToBoardMenu, setAddToBoardMenu] = useState<{
  x: number;
  y: number;
  file: FileData;
} | null>(null);
...
**Menu Positioning:** After "Add to Prompt", before "Copy Path"

**Constraints:**
- ✅ **Files only** (not directories)
- ✅ **Project must be selected** (needs project path for file data extraction)
- ✅ **Smart positioning** (context menu handles viewport edge detection)

**Lazy Loading:**
- **Boards:** Fetched on menu open (30s cache via `useBoards` hook)
- **Lists:** Fetched on board hover (30s cache via `useBoardHierarchy` hook)
- **Cards:** Fetched with lists (grouped by list ID)

**Success Feedback:**
```typescript
// Alert examples:
"Created board \"Button\" with file \"Button.tsx\""
"Created list \"helpers\" in board \"Project Alpha\""
"Created card \"Button\" in list \"To Do\""
"Attached \"Button.tsx\" to card \"Implement authentication\""
```

**Error Handling:**
- Network errors → Alert with error message
- Duplicate file attachment → Alert: `"File \"X\" is already attached to this card"`
- API failures → Alert with specific error

**Resource Sheet:** `coderef/resources-sheets/features/Add-File-To-Board-RESOURCE-SHEET.md` (800+ lines, comprehensive documentation)
...
**Modules Used:** architecture, integration, state-management, filtering, performance, move-operations, add-to-board-context-menu
**Complexity Score:** High (455 lines, recursive algorithms, hybrid routing, multiple view modes, move submenu system, board integration)
