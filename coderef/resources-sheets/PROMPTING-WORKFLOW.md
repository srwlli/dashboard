...
├── Corner Accents (decorative UI)
├── PromptSelector
│   ├── Prompt cards with tags
│   └── Tag filtering
├── AttachmentManager
│   ├── File drop zone
│   ├── Attachment list
│   └── Clear all button
├── WorkflowMeta
│   └── Workflow statistics (prompt info, attachment count)
├── ExportMenu
│   ├── Copy JSON button
│   ├── Export JSON button
│   └── Export Markdown button
├── Paste Final Result button
├── PasteFinalResultModal
│   └── Textarea for LLM output
└── ReviewTitleModal (conditional)
    └── Input for review title (prompt 0004 only)
```
...
**Rejected Because:**
- Workflow is simple (linear steps: select → attach → export → save)
...
```tsx
{!isReadyForExport && (
  <p>⚠️ Select a prompt and add at least one attachment to enable export</p>
)}
```
...
Show ReviewTitleModal
  ↓
User enters title → click Confirm
  ↓
...
- Modal confirms → need to know which export action to execute
...
5. Modal opens → user pastes response
...
4. User enters title → clicks Confirm
...
**Event Flow:**
```
[User Action] → [Handler] → [Context Update] → [Child Re-render]
[Export Action] → [Generate Data] → [Download/Clipboard] → [Done]
```
...
- If Electron directory selection fails → browser download fallback
- If clipboard API unavailable → manual copy-paste from export
...
- [ ] Test pending action queue (modal → export)
...
```typescript
const filename = `workflow_${workflow.selectedPrompt?.name}_${Date.now()}.json`;
// If name = "Feature/Bug Review" → invalid filename
```
