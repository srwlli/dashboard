...
- API routes act as thin adapters (HTTP → ScanExecutor)
...
**Guarantees:**
- ✅ Events emitted in order: 'output' → 'progress' → 'complete'/'error'
- ✅ `startScan()` only runs once (throws if status !== 'idle')
- ✅ Buffered output includes all historical output
- ✅ Auto-cleanup after 1 hour retention
- ❌ Does NOT guarantee subprocess success (depends on Python scripts)
- ❌ Does NOT guarantee all projects will be scanned (fail-safe skips errors)
...
**Isolation:**
- ✅ Each scan isolated by scanId (multiple concurrent scans possible)
- ✅ Per-project errors don't abort scan
- ✅ Auto-cleanup prevents memory accumulation (eventually)
...
- **Impact:** 5 projects × 60s each = 5 minutes total
...
- **Location:** `emitOutput()` → `emit('output')`
...
- **Reason:** SSE simpler for one-way streaming (server → client)
...
- [ ] Test dual-phase logic (scan → populate)
