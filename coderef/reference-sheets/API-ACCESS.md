...
- Future: Could add base64 → binary conversion if needed
...
**Next.js API Routes:**
- `GET /api/coderef/projects` → ProjectsApi.list()
- `POST /api/coderef/projects` → ProjectsApi.create()
- `DELETE /api/coderef/projects/[id]` → ProjectsApi.remove()
- `GET /api/coderef/tree?path=...` → TreeApi.load()
- `GET /api/coderef/file?path=...` → FileApi.load()
...
**Request Flow:**
```
[Component] → [CodeRefApi.projects.list()]
  ↓
...
- Component unmounts → API call completes → setState on unmounted component
